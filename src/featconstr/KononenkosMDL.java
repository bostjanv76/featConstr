...
public class KononenkosMDL extends Filter{
  ...
  protected double[][] m_CutPoints = null; //orig. protected
...
//        System.out.println("Teeeeeest"+i);
        if (copy == null) {
          copy = new Instances(data);
        }
        calculateCutPointsByMDL(i, copy);
//        System.out.println("Teeeestong");
      }
...
//XplainAttrConstr.print1d(counts[1]);
    // Save prior counts
...
//System.out.println("Prior counts: ");
//XplainAttrConstr.print1d(priorCounts);
...
//    System.out.println("bestCutPoint "+bestCutPoint);
//    System.out.println("bestEntropy "+bestEntropy);
    
//    System.out.println("bestCounts[0]: ");
//    XplainAttrConstr.print1d(bestCounts[0]);
//    System.out.println("bestCounts[1]: ");
//    XplainAttrConstr.print1d(bestCounts[1]);
...
/**
  ...
  public double kononenkosMDLDev(Instances instances,int attIndex, int first, int lastPlusOne) {
    double[][] counts, bestCounts;
    double[] priorCounts;
    double distPrior, instPrior, distAfter = 0, sum, instAfter = 0;
    double currentEntropy, bestEntropy, priorEntropy;
    double before, after;
    int numClassesTotal;
    double numInstances = 0;
...
//e.g. an instance with a weight of 2 corresponds to two identical instances with a weight of 1. (Instance weights are more flexible though because they don’t need to be integers.
//Note that any instance without a weight value specified is assumed to have a weight of 1 for backwards compatibility.
        counts[1][(int) instances.instance(i).classValue()] += instances.instance(i).weight(); //1d tabela kjer so frekvence posameznega razreda
    }
...
XplainAttrConstr.print1d(counts[1]);*/
    ...
    System.out.println("Number of diff num in attr: "+instances.attributeStats(attIndex).distinctCount);
    
    double [][]matrixCounts=new double[instances.numClasses()][instances.attributeStats(attIndex).distinctCount];
    for (int i = first; i < lastPlusOne; i++){
        matrixCounts[(int)instances.instance(i).classValue()][(int)instances.instance(i).value(attIndex)]++;        
    }
    
    System.out.println("Matrix counts");
    FeatConstr.print2d(matrixCounts);
/*System.out.println();
System.out.println("Prior counts: ");
XplainAttrConstr.print1d(priorCounts);*/
    // Entropy of the full set
    ...
    // Find best entropy.
    bestCounts = new double[2][instances.numClasses()];
    for (int i = first; i < (lastPlusOne - 1); i++){        //lastPlusOne - 1 because of later comparison if (instances.instance(i).value(attIndex) < instances.instance(i + 1).value(attIndex))
        counts[0][(int) instances.instance(i).classValue()] += instances.instance(i).weight();  //one minus class counts
        counts[1][(int) instances.instance(i).classValue()] -= instances.instance(i).weight();  //we are subtracting from class counts
        if (instances.instance(i).value(attIndex) < instances.instance(i + 1).value(attIndex)){
            currentEntropy = ContingencyTables.entropyConditionedOnRows(counts);        //Computes conditional entropy of the columns given the rows.
                if (currentEntropy < bestEntropy){
                    bestEntropy = currentEntropy;
                    System.arraycopy(counts[0], 0, bestCounts[0], 0,instances.numClasses());
                    System.arraycopy(counts[1], 0, bestCounts[1], 0,instances.numClasses());
    ...
    XplainAttrConstr.print2d(bestCounts);
    System.out.println();
    /*System.out.println("counts 2D: ");
    XplainAttrConstr.print2d(counts);*/
    ...
    // Encode distribution prior to split
    distPrior = SpecialFunctions.log2Binomial(numInstances + numClassesTotal - 1, numClassesTotal - 1);
        //System.out.println("distPrior "+distPrior);
    // Encode instances prior to split.
    instPrior = SpecialFunctions.log2Multinomial(numInstances, priorCounts);
        //System.out.println("instPrior "+instPrior);
    ...
    for (double[] bestCount : matrixCounts) {
        ...
        distAfter += SpecialFunctions.log2Binomial(sum + numClassesTotal - 1, numClassesTotal - 1);
        //System.out.println("distAfter "+distAfter);
        instAfter += SpecialFunctions.log2Multinomial(sum, bestCount);
    ...
    //gain =priorEntropy - bestEntropy)
 //System.out.println("MDL\': "+((priorEntropy - bestEntropy)+((distPrior-distAfter)/numInstances)));   //???
    return (before-after)/numInstances;

  }
   
  public double kononenkosMDL(Instances instances,int attIndex) {
    //double[] priorCounts= new double[instances.attributeStats(instances.classIndex()).distinctCount];
    double distPrior, instPrior, distAfter = 0, sum, instAfter = 0;
    double before, after;
    int numClassesTotal;
    double numInstances = 0;
    ...
    /*System.out.println("priorCounts counts");
    XplainAttrConstr.print1d(priorCounts);*/
    
    /*System.out.print("Indeks atributa: "+attIndex);System.out.print(" Ime atributa: "+instances.attribute(attIndex).name());
    System.out.print(" Zaloga vrednosti tributa: "+instances.attributeStats(attIndex).distinctCount);
    System.out.println(" Število vrednosti razreda: "+instances.numClasses());*/
    //double [][]matrixCounts=new double[instances.numClasses()][instances.attributeStats(attIndex).distinctCount];
    ...
    //double [][]matrixCounts=new double[instances.attributeStats(attIndex).distinctCount][instances.numClasses()]; //old ... not good
...
matrixCounts[(int)instances.instance(i).value(attIndex)][(int)instances.instance(i).classValue()]++;
        //matrixCounts[instances.instance(i).attribute(attIndex).indexOfValue(""+(int)instances.instance(i).value(attIndex))][(int)instances.instance(i).classValue()]++;
        
        numInstances += instances.instance(i).weight();          
    }
    
    /*System.out.println("Matrix counts "+ "attr: "+instances.attribute(attIndex).name());
    XplainAttrConstr.print2d(matrixCounts);*/
...
/*
 MDL naj za številske atribute ne uporablja diskretizacije, ampak v zanki preizkusi vse smiselne meje med sortiranimi vrednostmi in nato oceni dvorazredni atribut. Tako deluje npr .moja implementacija (funkcija estimation::impuritySplit) v R paketu CORElearn. 
  */
  ...
  //double estimation::impuritySplit(construct &nodeConstruct, double &bestEstimation){
   ...
   //ArrayList<Integer>noClassAttrVal[]=new ArrayList[2];
  ...
  //List<List<Double>> NumValues = new ArrayList<List<Double>>();  
  //ArrayList<SortRec> sortedMean = new ArrayList<SortRec>(); 
   ...
   //for (j=0 ; j < TrainSize ; j++){
    for (j=0; j<trainSize;j++){   
      ...
      attrValue=data.instance(j).value(idx);
        ...
        if (Double.isNaN(attrValue)) //if (isNAcont(NumValues(j,i)))  int isNAcont(x) -> R_IsNA(x) ... kontroling missing value
            continue;
        sortedAttr.get(OKvalues).setKey(attrValue);           //sortedAttr[OKvalues].key = NumValues(j,i); 
            //sortedAttr[OKvalues].key = attrValue;
        sortedAttr.get(OKvalues).setValue(j);
            //sortedAttr[OKvalues].value = j ;
      //System.out.println((int) data.instance(j).classValue());
            //noClassAttrVal(DiscValues(j, 0), 2)++ ; // initially we store all for the right side
        noClassAttrVal[(int) data.instance(j).classValue()][2]++; //DiscValues.get(j).get(i);    //DiscValues(j,i); ... classValue = DiscValues(j, 0)  
      OKvalues++ ;
   }
   ...
   //double priorImpurity = (this->*fImpurity)(OKvalues, noClassAttrVal, 2) ; //fImpurity = &estimation::MDLimpurity ;
    ...
    for (int i = 0; i < trainSize; i++) {     // orig. for (int i = 0; i < trainSize+1; i++) {
...
//XplainAttrConstr.print1d(priorCounts);
   ...
   //sortedAttr.setFilled(OKvalues) ;   //sets the point to wich the array is filled
   //sortedAttr.qsortAsc() ;
   ...
   for (j=0 ; j < minNodeWeightEst ; j++) {
       noClassAttrVal[(int) data.instance(j).classValue()][1]++;
